...
//  ver10. 예외처리클래스를 구현함으로써 예외처리를 강화하였고 코드의 가독성과 유지보수성을 높였습니다.
//
...
private:
    int RequireDeposit;
public:
    DepositException(int money) : RequireDeposit(money) {}
    void ExceptionMessage()
    {
        cout<<"[*입금 오류]"<<endl;
        cout<<"[에러메시지 : "<<RequireDeposit<<"원을 입금할 수 없습니다.]"<<endl;
        cout<<"입금액을 다시 입력해주세요."<<endl;
    }
};
...
public:
    ...
    {
        cout<<"[*출금 오류]"<<endl;
        cout<<"[에러메세지 : 잔액이 부족합니다.]"<<endl;
        cout<<"[잔액 : "<<balance<<"원]"<<endl;
        cout<<"출금액을 다시 입력해주세요."<<endl;
    }
};
...
public:
    ...
    void DepositMoney() throw (AccountException);
    void WithdrawMoney() throw (AccountException);
...
void AccountHandler::DepositMoney() throw (AccountException)
{
    ...
    while(1)
    {
        ...
        cout<<"(메인메뉴로 돌아가길 원하시면 -1을 입력해주세요.)"<<endl;
    }
}
void AccountHandler::WithdrawMoney() throw (AccountException)
{
    int myaccount = 0, money = 0;
    cout<<"[출   금]"<<endl;
    cout<<"계좌번호 : ";
            ...
            for(int i = 0; i < AccountNumberCount; i++)
            {
                if(customer[i]->GetAccount() == myaccount)
                {
                    if(customer[i]->Withdraw(money) == 0)
                        throw WithdrawException(customer[i]->GetBalance());
